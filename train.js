...
var txp = 0; // координата х нажатая пользователем
var typ = 0; // координата у нажатая пользователем
var x2_n = 0; // следующий второй множитель

stop(x1, x2); // Функция очистки и перерисовки экрана	
...
// функция нажатия мышкой и проверки соответствия координаты точке
canvas.addEventListener("mousedown", n_t, false);
function n_t(e) {
	if (x1 != 0 && x2 != 0) { // не равны нулю множители
		x2_n = x2_n + 1;
		if (x1 * x2_n <= 99) { // разрешено обводить фигуру для произведений, меньше 100			
			pr = x1 * x2_n;			
			pr_d_e(pr);
			txp = e.clientX - canvas.offsetLeft;
			typ = e.clientY - canvas.offsetTop;
			if (Math.abs(tx[pr_e] - txp) <= 30 && Math.abs(ty[pr_e] - typ) <= 30) {
				context.fillStyle = "black"	;				
				context.textBaseline="alphabetic";
				context.font = "30pt Courier New monospace";
				context.clearRect(351, 41, 48, 48); // очистить поле				
				context.fillText(x2_n, 375, 80); // второй множитель				
				if (x1 == 5) {
				...
				alert("ВЫ ОШИБЛИСЬ! НАЧНИТЕ С САМОГО НАЧАЛА!");
				stop(x1, x2);
				radio1.checked = false;
			}
			if (x2_n == x2) {
			...
			alert("ОТЛИЧНО! ПОТРЕНИРУЙСЯ И С ДРУГИМИ МНОЖИТЕЛЯМИ!");
			stop(x1, x2);
			radio1.checked = false;
		}
...
function check() { 
	if (radio1.checked) { // показать подсказку
		pr_p = pr; // запоминаем, до какого значения произведения дошел пользователь
		hint_yes(pr_p);		
	}
	else if (radio2.checked) { // скрыть подсказку
		pr_p = pr;
		hint_no(pr_p);
	}
}

function hint_yes(pr_p) { // показать подсказку
	if (x1 != 0 && x2 != 0) { // не равны нулю множители
		x2 = 0;
		stop(x1,x2); // очистка и перерисовка экрана
		abris(x1,x2); // рисуем черный абрис в соответствии с первым множителем
		abris_p(x1,x2); // перерисовываем, что успел нарисовать пользователь	
		x2 = combo2.value;
	}
	else {
		alert("Нажмите кнопку 'УМНОЖЬТЕ' для выбора множителей!");
...
function hint_no(pr_p) { // скрыть подсказку
	x2 = 0;
	stop(x1,x2); // очистка и перерисовка экрана
	abris_p(x1,x2); // перерисовываем, что успел нарисовать пользователь
	x2 = combo2.value;
...
function abris(x1,x2) { // рисуем черный абрис в соответствии с первым множителем
	for(var n = 1; n <= 10; n++) {
		x2 = x2 + 1;
		pr = x1 * x2;
		pr_d_e(pr);
...
function abris_p(x1,x2) { // перерисовываем, что успел нарисовать пользователь
	x2_n = 0;
	pr = 0;	
	while (pr < pr_p) {
		x2_n = x2_n + 1;
